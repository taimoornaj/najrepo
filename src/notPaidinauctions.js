...
export default class NotPaidinauctions extends Component {
    constructor(props){
        super(props);
        status_payment = new Array();
        status_payment[0] = I18n.t('car.all');
        status_payment[1] = I18n.t('car.paid');
        status_payment[2] = I18n.t('car.unpaid');
        this.state = {
            setd: '',
            search: '',
            allcount: 0,
        ...
        this.setState(
            {
                ...
                to_date: new Date(new Date().getFullYear(), 1, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 1,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 1, 1),
                to_date: new Date(new Date().getFullYear(), 2, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 2,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 2, 1),
                to_date: new Date(new Date().getFullYear(), 3, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 3,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 3, 1),
                to_date: new Date(new Date().getFullYear(), 4, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 4,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 4, 1),
                to_date: new Date(new Date().getFullYear(), 5, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 5,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 5, 1),
                to_date: new Date(new Date().getFullYear(), 6, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 6,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 6, 1),
                to_date: new Date(new Date().getFullYear(), 7, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 7,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 7, 1),
                to_date: new Date(new Date().getFullYear(), 8, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 8,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 8, 1),
                to_date: new Date(new Date().getFullYear(), 9, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 9,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 9, 1),
                to_date: new Date(new Date().getFullYear(), 10, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 10,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 10, 1),
                to_date: new Date(new Date().getFullYear(), 11, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
        ...
        this.setState(
            {
                value: 11,
                previous_balance: 0,
                from_date: new Date(new Date().getFullYear(), 11, 1),
                to_date: new Date(new Date().getFullYear(), 12, 1)
            },
            this.getData         // here is where you put the callback
        );
    }
    ...
    getData = () => {
        ...
        var date_from   = (this.state.from_date)?this.state.from_date:'';
        var date_to     = (this.state.to_date)?this.state.to_date:'';
        var Url  = "https://api.nejoumaljazeera.co/api/carStatement/InAuctionCarsnoAuth?customer_id="+AuthContext.id+
    ...
    render(){
        ...
        /**if(this.state.from_date != '' || this.state.to_date != ''){
            var date_scale = <View style={{flexDirection:'row', flex:1, justifyContent:'space-between', alignItems:'center'}}>
                                    <View style={{flex:0.4}}>
                                        <Text style={{color:'#1760B2'}}>{this.state.from_date.toISOString().replace(/T/, ' ').replace(/\..+/, '').substring(0, 10)}</Text>
                                    </View>
                                    <View style={{flex:0.1}}>
                                        <FontAwesomeIcon 
                                            icon={faArrowRight}
                                            size={20} backgroundColor="#fff" color="#343D40"/> 
                                    </View>
                                    <View style={{flex:0.4}}>
                                        <Text style={{color:'#1760B2'}}>{this.state.to_date.toISOString().replace(/T/, ' ').replace(/\..+/, '').substring(0, 10)}</Text>
                                    </View>
                                        <TouchableOpacity activeOpacity={1} style={{flex:0.1}} onPress={() => this.resetDate()}>
                                            <View>
                                                <FontAwesomeIcon 
                                                    icon={faX}
                                                    size={20} color='#343D40' backgroundColor="#343D40" />
                                            </View>
                                        </TouchableOpacity>
                                </View>;
        }else {
...
<DatePicker
                        ...
                        />
                                    ...
                                    <View style={{flexDirection: 'row'}}>
                                        <Text style={styles.whiteText2}>{strings('main.transfer')}</Text>
                                    </View>
                                        <RNPicker
                    ...
                    <View style={{flexDirection:(I18n.locale == 'ar')?'row-reverse':'row', justifyContent:'space-around', margin:'2%'}}>
                        ...
                        <Text style={{textAlign:'center', color:'#a30000', fontSize: width*0.05}}>{this.state.balance} AED</Text>
                    </View>
                                ...
                                return(
                                    <View style={{margin:'2%', flexDirection:'row', borderBottomWidth: 1, justifyContent:'center', borderColor: '#ccc'}}>
                                        <View style={{flex:0.35, justifyContent:'center', alignItems:'center', flexDirection:'column'}}>
                                            <Text style={{color:'#343D40', fontSize:width*0.03}}>{item.description}</Text>
                                            <Text style={{color:'#343D40', fontSize:width*0.03}}>{item.date}</Text>
                                        </View>
