...
const PaymentOther = props => {
  const baseUrl = `https://api.nejoumaljazeera.co/api/getAllOnlinePaymentOther?customer_id=${
    AuthContext.id
  }`;
  ...
  const getDetails = async (paymentId) => {
    ...
    var Url = "https://api.nejoumaljazeera.co/api/getPaymentOtherDetails?payment_id="+paymentId;
    try {
      fetch(Url, {
        method: 'GET',
    ...
    return (
      <View style={localStyles.itemStyle} key={payment_id}>
        <View
          ...
          <Text style={localStyles.oldPort}>From: {(AccountName)?(I18n.locale == 'en')?AccountName:AccountName:''}</Text>
          ...
          <TouchableOpacity
              ...
              ><Text style={localStyles.newPort}>To: {cars_count} {strings('main.services')} </Text>
              <FontAwesomeIcon
                  icon={ faInfoCircle }
                  color="#013188"
                  style={{marginTop: 5 }}
  ...
  const renderItemDetails = ({index, item}) => {
    var dataDetails = {};
    dataDetails.id              = item.id;
    const actions={}
    return (
      <View style={{flex:1}}>
        <Text style={{textAlign:I18n.local == 'en'?'left':'right'}}>{++index} - {(I18n.locale == 'en')?item.description:item.description_ar}</Text>
      </View>
    );
  };
              ...
              {!loaderInside?
                ...
                <View style={[localStyles.content]}>
                 <View style={{flexDirection:'row', margin:'3%'}}>
                      <Text style={[commonStyle.headerText, {color: '#0b4282'}]}>{strings('car.details')}</Text>
                  </View>
